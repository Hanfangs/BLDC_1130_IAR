...
/**
  ...
  * @retval None
  */
static void TI1_Config(TIM_TypeDef* TIMx, uint16_t TIM_ICPolarity, uint16_t TIM_ICSelection,   ///这个极性为什么是0x0A  双极性，照理来说好象只能支持上升或下降啊？？？？
                       uint16_t TIM_ICFilter)
{
     ...
     (TIMx == TIM4) ||(TIMx == TIM5))
  {
    ...
    tmpccer &= (uint16_t)~((uint16_t)(TIM_CCER_CC1P));                      ///  TIM_CCER_CC1P = 0x02
    tmpccer |= (uint16_t)(TIM_ICPolarity | (uint16_t)TIM_CCER_CC1E);         ///TIM_ICPolarity = 0x0A  怎么理解  TIM_ICPolarity_BothEdge=0x000A ?
  }
  else
  {
    /* Select the Polarity and set the CC1E Bit */
    tmpccer &= (uint16_t)~((uint16_t)(TIM_CCER_CC1P | TIM_CCER_CC1NP));
    tmpccer |= (uint16_t)(TIM_ICPolarity | (uint16_t)TIM_CCER_CC1E);          /// TIM_CCER_CC1E = 0x01
  }
